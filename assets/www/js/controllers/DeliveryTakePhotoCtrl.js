app.controller('DeliveryTakePhotoCtrl', function($scope, $ionicHistory, $ionicPopup, $ionicModal, $state, SessionService, camera,APIService,$localStorage,PageService,DeviceService,Languages,Deviceconfig,WLog,ShipmentAPI,Spots,PageService) {
    ...
    DeviceService.chk_device_status(function(net_status, gps_status, box_status) {
        ...
        if ($scope.device_status[0] == 1 && $scope.device_status[1] == 1 && $scope.device_status[2] == 1) {
            ...
            APIService.AppConfig.getchktom(function(TOMs_status) {
                    ...
                    console.log('######## TM18 TOMs_status :' + $scope.TOMs_status);
                })
    ...
    Languages.getDBlang($localStorage.lang_id, 'TM18', function(result) {
    ...
    $scope.DeliveryTakePhotoCancel = function() {
        // reset
        JSON_D17_Pre={
            ReturnCode:"1", 
            LogImage:[]
    ...
    function createConfirmModal() {
        $ionicModal.fromTemplateUrl('delivery-confirm.html', {
            scope: $scope,
    ...
    $scope.DeliveryTakePhotoSave = function() {
        $scope.modal.show();
        $('#delivery-confirm__modal').velocity({
            opacity: [1, 0]
        }, {
            duration: 300
        });
    ...
    $scope.confirm = function() {
        ...
        Spots.updateDBspot("spot_status", $localStorage.ship_id, $localStorage.ship_spot_id, "SUC", function(updateDBspot_status){
        ...
        $('.delivery, #delivery-confirm__modal').velocity({
            ...
            complete: function() {
                ...
                // Page Config
                $scope.page = PageService.find_page('TM21');

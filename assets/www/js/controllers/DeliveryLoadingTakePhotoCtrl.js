app.controller('DeliveryLoadingTakePhotoCtrl', function($scope, $ionicHistory, $ionicPopup, $ionicModal, $state, SessionService, camera,APIService,$localStorage,PageService,DeviceService,Languages,Deviceconfig,WLog,ShipmentAPI,Spots) {
// Check Session login
    ...
    DeviceService.chk_device_status(function(net_status, gps_status, box_status) {
        ...
        if ($scope.device_status[0] == 1 && $scope.device_status[1] == 1 && $scope.device_status[2] == 1) {
            ...
            APIService.AppConfig.getchktom(function(TOMs_status) {
                    ...
                    console.log('######## TM24 TOMs_status :' + $scope.TOMs_status);
                })
    ...
    Languages.getDBlang($localStorage.lang_id, 'TM24', function(result) {
    ...
    $scope.deliveryLoadingTakePhotoSave = function() {

        $scope.modal.show();
        $('#delivery-loading-confirm__modal').velocity({
            opacity: [1, 0]
        }, {
            duration: 300
        });
    ...
    $scope.deliveryLoadingTakePhotoCancel = function() {
        // reset
        JSON_D17_Pre={
            ReturnCode:"1", 
            LogImage:[]
    ...
    function createConfirmModal() {
        $ionicModal.fromTemplateUrl('delivery-loading-confirm.html', {
            scope: $scope,
    ...
    $scope.confirm = function() {
        ...
        Spots.updateDBspot("spot_status", $localStorage.ship_id, $localStorage.ship_spot_id, "EUC", function(updateDBspot_status){
        ...
        $('#delivery-loading-confirm__modal').velocity({
            ...
            complete: function() {
                ...
                setTimeout(function() {
                    $scope.modalBroken.show();
                    $('#check-broken-confirm__modal').velocity({
                        opacity: [1, 0]
                    }, {
                        duration: 300
                    });
    ...
    // Check Broken Modal
    function createCheckBrokenModal() {
        $ionicModal.fromTemplateUrl('check-broken-confirm.html', {
            scope: $scope,
            animation: 'slide-in-down'
        }).then(function(modal) {
            $scope.modalBroken = modal;
        });
    }
    createCheckBrokenModal();

    $scope.brokenYes = function() {
        $('.delivery-loading, #check-broken-confirm__modal').velocity({
            opacity: 0
        }, {
            duration: 300,
            complete: function() {
    ...
    $scope.brokenNo = function() {
        $('#check-broken-confirm__modal').velocity({
            ...
            complete: function() {
                ...
                // Next Page Config
                Spots.getDBnext_page($localStorage.ship_id, $localStorage.ship_spot_id, $localStorage.OrderRankSc+1, function(result){
                    //$localStorage.OrderRankSc = ;
                    console.log("--------------------getDBnext_page ----------------------");
                    console.log("-------------------- scenario "+result[0].spotc_scenario);
                    $localStorage.OrderRankSc = result[0].spotc_seq;

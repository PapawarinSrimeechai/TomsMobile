app.controller('PickupLoadingTakePhotoCtrl', function($scope, $ionicHistory, $ionicPopup, $ionicModal, $state, SessionService, camera,APIService,$localStorage,PageService,DeviceService,Languages,Deviceconfig,WLog,ShipmentAPI,Spots) {
    ...
    // local Strorage
    $localStorage.lang_page = "TM12";
    $localStorage.spotc_scenario = 5;
    ...
    DeviceService.chk_device_status(function(net_status, gps_status, box_status) {
        ...
        if ($scope.device_status[0] == 1 && $scope.device_status[1] == 1 && $scope.device_status[2] == 1) {
            ...
            APIService.AppConfig.getchktom(function(TOMs_status) {
                    ...
                    console.log('######## TM012 TOMs_status :' + $scope.TOMs_status);
                })
    ...
    Languages.getDBlang($localStorage.lang_id, 'TM12', function(result) {
    ...
    $scope.PickupLoadingTakePhotoCancel = function() {
    ...
    function createConfirmModal() {
        $ionicModal.fromTemplateUrl('pickup-loading-confirm.html', {
            scope: $scope,
    ...
    $scope.PickupLoadingTakePhotoSave = function() {

        $scope.modal.show();
        $('#pickup-loading-confirm__modal').velocity({
            opacity: [1, 0]
        }, {
            duration: 300
        });
    ...
    Spots.updateDBspot("spot_status", $localStorage.ship_id, $localStorage.ship_spot_id, "ELC", function(updateDBspot_status){
    ...
    $scope.confirm = function() {
        ...
        $('.pickup-loading, #pickup-loading-confirm__modal').velocity({
            opacity: 0
        }, {
            duration: 300,
            complete: function() {

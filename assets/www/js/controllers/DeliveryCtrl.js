app.controller('DeliveryCtrl', function($scope, $ionicHistory, $ionicPopup, $ionicModal, $state,SessionService,APIService,$localStorage,PageService,DeviceService,Languages,Deviceconfig,WLog,ShipmentAPI,Spots,PageService) {
    ...
    DeviceService.chk_device_status(function(net_status, gps_status, box_status) {
        ...
        if ($scope.device_status[0] == 1 && $scope.device_status[1] == 1 && $scope.device_status[2] == 1) {
            ...
            APIService.AppConfig.getchktom(function(TOMs_status) {
                    ...
                    console.log('######## TM17 TOMs_status :' + $scope.TOMs_status);
                })
    ...
    Languages.getDBlang($localStorage.lang_id, 'TM17', function(result) {
    ...
    $scope.confirm = function() {
        ...
        Spots.updateDBspot("spot_status", $localStorage.ship_id, $localStorage.ship_spot_id, "SUS", function(updateDBspot_status) {
        ...
        $('.delivery, #delivery-confirm__modal').velocity({
            ...
            complete: function() {
                ...
                // Next Page Config
                $scope.page = PageService.find_page('TM18');
                $state.go($scope.page);
            }
        });
    };
